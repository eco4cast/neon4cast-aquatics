...
cur_neon_store_month <- format(as.Date(max(wq_cleaned$time)), "%Y-%m")
# what is the next month from this plus the current month? These might be the same
new_months <- unique(format(c((as.Date(max(wq_cleaned$time)) %m+% months(1)), Sys.Date()), "%Y-%m"))



for (i in 1:length(current_sites)) {
  # create a directory for each site (if one doesn't already exist)
  if (current_sites[i] %in% list.dirs(download_location, full.names = F)) {
    # message('directory exists')
  } else {
    dir.create(path = paste0(download_location, current_sites[i]))
