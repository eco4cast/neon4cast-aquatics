...
Add observation error to the forecast
...
model_output %>% 
  ...
  summarise(mean = mean(oxygen),
            upper_latent = quantile(oxygen, 0.975),
            lower_latent = quantile(oxygen, 0.025),
            upper_obs = quantile(oxygen_w_obs_error, 0.975),
            lower_obs = quantile(oxygen_w_obs_error, 0.025), .groups = "drop") %>% 
  ggplot(aes(x = time, y = mean)) +
  geom_line() +
  geom_ribbon(aes(ymin = lower_latent, ymax = upper_latent), alpha = 0.2, color = "lightblue", fill = "lightblue") +
  geom_ribbon(aes(ymin = lower_obs, ymax = upper_obs), alpha = 0.2, color = "pink", fill = "pink") +
...
dat <- forecast %>% 
  select(time, oxygen_w_obs_error, ensemble) %>% 
  pivot_wider(names_from = ensemble, values_from = oxygen_w_obs_error) %>% 
  select(-time)
